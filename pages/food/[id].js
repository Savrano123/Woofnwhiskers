...
export default function FoodProductPage({ product, relatedProducts = [] }) {
          ...
          <div className="mt-12">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {relatedProducts.map(relatedProduct => (
                <Link href={`/food/${relatedProduct.id}`} key={relatedProduct.id}>
                  <a className="bg-white rounded-lg shadow-md overflow-hidden transition-transform hover:scale-105">
                    ...
                    <div className="p-4">
                      ...
                      <p className="text-gray-600 mb-1">{relatedProduct.brand}</p>
...
export async function getStaticPaths() {
  // Fetch all food product IDs from the database
  let products = [];
  ...
  try {
    ...
    console.error('Error fetching food products for static paths:', error);
...
export async function getStaticProps({ params }) {
  ...
  try {
    ...
    relatedProducts = allProducts
      .filter(p =>
        (p.category === 'Food' || p.category?.toLowerCase() === 'food') &&
        p.id !== product.id &&
        p.petType === product.petType
      )
      .slice(0, 4);

    // If no related products, get any food products
    ...
    product = {
      ...
      name: ['Premium Dry Dog Food', 'Kitten Wet Food', 'Senior Dog Food', 'Adult Cat Dry Food'][parseInt(params.id) - 1] || 'Pet Food',
      brand: ['Royal Canin', 'Whiskas', 'Pedigree', 'Meow Mix'][parseInt(params.id) - 1] || 'Unknown',
      petType: ['Dog', 'Cat', 'Dog', 'Cat'][parseInt(params.id) - 1] || 'Unknown',
      description: 'This high-quality pet food provides complete and balanced nutrition for your pet. Made with premium ingredients and essential vitamins and minerals to support overall health and wellbeing.',
      price: [1200, 450, 950, 800][parseInt(params.id) - 1] || 500,
      weight: ['5kg', '400g', '3kg', '2kg'][parseInt(params.id) - 1] || '1kg',
